...
const SuggestedBuisness = ({buisness}) => {
  ...
  const getBuisnessList = () => {
    GlobalApi.getBuisnessListByCategory(buisness?.category?.name).then((res) => {
    ...
    <div className="pl-10">
      ...
      <div >
        {buisnessList && buisnessList.map((item,index)=>(
          <Link href={'/details/'+item.id} key={index} className="flex gap-2 mt-3 mb-3 hover:border border-violet-700 rounded-lg p-2 cursor-pointer hover:shadow-md">
