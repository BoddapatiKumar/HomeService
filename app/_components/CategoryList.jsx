    ...
    <div className="mx-4 md:mx-22 lg:mx-52 grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
        {
           Categories.length>0 ? Categories.map((item,index)=>(
                <Link href={'/search/'+item.name} key={index} className="flex flex-col items-center justify-center gap-2 p-5 rounded-lg cursor-pointer hover:scale-110 transition-all ease-in-out bg-slate-100/80"
               >
